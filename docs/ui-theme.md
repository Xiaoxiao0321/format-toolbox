...
后续根据使用反馈修正了分隔线与列表：拖动手柄直接更新设置宽度及工作区／结果区比例，小幅拖动即时生效，超出范围时停在边界；三个列表使用各自的纵向与横向滚动条。历史记录的四个操作按钮位于列表右侧竖排，列表的固定列宽由自身横向滚动条承载，不会挤掉按钮。“更多功能”菜单明确加载共享菜单样式，统一 14 DIP 字体、青蓝悬浮色与 8 DIP 圆角；下拉选项字体和标签选中字体的继承也已修正。

默认宽窗口的工作区／结果区高度从 7:3 微调为 6.4:3.6，结果区最小高度为 230 DIP，宽布局内容最小高度为 610 DIP。主内容与菜单的上间距从 8 缩至 2 DIP，快速入口与文件／设置工作区的间距从 8 缩至 4 DIP；用户拖动后仍保留自定比例。
